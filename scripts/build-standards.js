...
function render(subjects, byLevel) {
  return [
    ...
    ...['elementary', 'middle', 'high'].map((level) => `  ${level}: '${stableJson(byLevel[level])}',`),
    '});',
    '',
  ].join('\n');
}
...
function build(options) {
  ...
  try {
    ...
    fs.writeFileSync(options.output, render(subjects, byLevel), 'utf8');
