...
// Achievement standards live in one file per school level (StandardsData_
// elementary/middle/high.gs) so Apps Script doesn't have to parse all ~29MB
// on every request when only one level's data is ever read at a time.
const LEVEL_FILES = {
  elementary: 'StandardsData_elementary.gs',
  middle: 'StandardsData_middle.gs',
  high: 'StandardsData_high.gs',
};
const LEVEL_CONSTANTS = {
  elementary: 'STANDARDS_DATA_ELEMENTARY',
  middle: 'STANDARDS_DATA_MIDDLE',
  high: 'STANDARDS_DATA_HIGH',
};
...
function loadExisting(dir) {
  ...
  return Object.entries(LEVEL_FILES).flatMap(([schoolLevel, fileName]) => {
    const file = path.join(dir, fileName);
    const source = fs.readFileSync(file, 'utf8');
    const context = {}; vm.createContext(context);
    vm.runInContext(`${source}\nthis.rows=JSON.parse(${LEVEL_CONSTANTS[schoolLevel]});`, context, { filename: file });
    return context.rows
...
function render(records) {
  ...
  return {
    // STANDARDS_SUBJECTS is small and always needed, so it stays in
    // StandardsData.gs. Each level's achievement standards are only read
    // when an admin reloads that level's data, so they get their own file
    // (see CentralData.gs's getBundledStandardsRaw_).
    subjects: `${HEADER}\nconst STANDARDS_SUBJECTS = '${json(subjects)}';\n`,
    elementary: `${HEADER}\nconst STANDARDS_DATA_ELEMENTARY = '${json(data.elementary)}';\n`,
    middle: `${HEADER}\nconst STANDARDS_DATA_MIDDLE = '${json(data.middle)}';\n`,
    high: `${HEADER}\nconst STANDARDS_DATA_HIGH = '${json(data.high)}';\n`,
  };
}
...
function build({ sourceDir, existing, output, referenceBook5 }) {
  ...
  try {
    ...
    const enriched = enrich([...loadExisting(path.dirname(existing)), ...book4Records, ...specialistRecords]);
    ...
    fs.writeFileSync(output, rendered.subjects, 'utf8');
    fs.writeFileSync(path.join(outputDir, LEVEL_FILES.elementary), rendered.elementary, 'utf8');
    fs.writeFileSync(path.join(outputDir, LEVEL_FILES.middle), rendered.middle, 'utf8');
    fs.writeFileSync(path.join(outputDir, LEVEL_FILES.high), rendered.high, 'utf8');
