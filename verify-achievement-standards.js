...
for (const level of levels) {
  ...
  for (const record of context.data[level]) {
    if (record.code && !codePattern.test(record.code)) throw new Error(`unsupported code: ${record.code}`);
    if (!record.subject || !record.text || !record.text.trim() || !record.curriculumRevision || !record.subjectKey || !record.standardKey) throw new Error(`missing metadata/subject/text: ${record.code}`);
    if (!['2015', '2022'].includes(String(record.curriculumRevision))) throw new Error(`invalid curriculum revision: ${record.curriculumRevision}`);
    const key = [level, record.curriculumRevision, record.track || '', record.subject, record.domain || '', record.code, record.text].join('\0');
...
for (const subject of context.subjects) {
  if (!levels.includes(subject.schoolLevel) || !subject.name || !subject.curriculumRevision || !subject.subjectKey) throw new Error(`invalid subject catalog item: ${JSON.stringify(subject)}`);
}
...
if (!context.subjects.some((subject) => subject.schoolLevel === 'high'
    && subject.curriculumRevision === '2015'
    && subject.category === '보통교과'
    && subject.name === '실용 국어')) {
  throw new Error('2015 high-school common subject missing: 실용 국어');
}
if (!context.data.high.some((record) => record.curriculumRevision === '2015' && !record.code)) throw new Error('2015 code-less 전문교과Ⅱ standards missing.');
